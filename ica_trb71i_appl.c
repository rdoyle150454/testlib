...
*   ADC-מ אצוי םירבוצ קשממ טלק ץבוק
    SELECT K_TRB71I_ADC
           ASSIGN                 TO           "SYS$DISK"
...
*   תואיגש לש הרקמב ל"אוד תעדוהל טלפ ץבוק
...
*--------------------------------------------------------------------
 ...
 FD P_TRB71I_ATR_R  IS EXTERNAL
    VALUE OF ID IS L$_P_TRB71I_ATR_R_NAME.
 ...
 01 P_TRB71I_MSM_MSG_REC		PIC X(140).
...
* םיינוציח םיצבק תומש
 ...
 01 L$_WS01_FILES_NAME.
    ...
    03 L$_P_TRB71I_875_NAME_X.
       ...
       05 FILLER		    PIC X(15)	    VALUE "ICA_TRB71I_875_".
       05 L$_WS01_TRNF_ID	    PIC 9(06).
       05 FILLER		    PIC X(04)	    VALUE ".DAT".
    ...
    03 L$_P_TRB71I_ATR_F_NAME.
       05 FILLER		    PIC X(15)	    VALUE "ICA_TR_DAT_DIR:".
       05 FILLER		    PIC X(17)	    VALUE "ICA_TRB71I_ATR_F_".
       05 L$_WS01_TRNF_ID	    PIC 9(06).
       05 FILLER		    PIC X(04)	    VALUE ".DAT".
 ...
 01 L$_WS01_VARAIBLES.
    ...
    03 L$_CRNT_EXT_ID_NUM_TYP_COD_X  REDEFINES
 ...
 01 L$_CO01_CONSTANTS.
    03 L$_CO01_DELAY_DEADLOCK	    PIC X(23)	    VALUE "0 00:00:01.00".
 ...
 01 L$_CT01_COUNTERS.
    ...
    03 L$_CT01_INP_TRL_REC	    PIC 9(10)	    VALUE 0.
 ...
 01 L$_MH01_KEYS_TO_COMPARE.
    03 L$_MH01_CURR_CUST.
	  ...
	  07 FILLER                     PIC X(11).
       ...
       05 L$_MH01_CURR_CUST_INT_ID	PIC 9(09).
...
*------------------------
 ...
 COPY "ICA_SOURCE:ICA_HIGH_LOW_VAL_TYPES.INC".
...
*-----------------
 ...
 COPY "ICD_CDD_WKSP:ICD_SVT_030_ENTITY_DBW"	     FROM DICTIONARY.
...
****
    IF NOT SP$_COBRMS_SUCCESS IN SP$_COBRMS_FILE_STATUS
       DISPLAY "** Error handling in file: " L$_K_TRB71I_ADC_NAME
...
*-----------------------------------------------------------------------
    USE AFTER STANDARD ERROR PROCEDURE ON P_TRB71I_ATR_R.
 015.
****
    IF NOT SP$_COBRMS_SUCCESS IN SP$_COBRMS_FILE_STATUS
...
*----------------------------------------------------------------------
 BA-00.
    ...
    IF SP$_SYSPRO_STATUS  OF UTL_DECLARE_EXIT_HANDLER_WKSP  IS FAILURE
       MOVE SP$_SYSPRO_STATUS	    IN UTL_DECLARE_EXIT_HANDLER_WKSP 
...
*----------------------------------------------------------------------
 BB-00.
    ...
    CALL "UTL_TRAN_LOGICAL_NAME_V2"  USING  UTL_TRAN_LOGICAL_NAME_WKSP
    ...
    IF SP$_SYSPRO_STATUS  OF UTL_TRAN_LOGICAL_NAME_WKSP  IS FAILURE
       ...
       DISPLAY "STTSA=", 
               SP$_SYSPRO_STATUS_AUXIL  IN UTL_TRAN_LOGICAL_NAME_WKSP CONVERSION
    ...
    MOVE SP$_LOGICAL_NAME_VALUE OF UTL_TRAN_LOGICAL_NAME_WKSP  
      TO L$_ICA$ENV
...
*      Get previous oper id.
       MOVE "PREV_OPER_ID" TO SP$_SYMBOL IN UTL_GET_SYMBOL_VALUE_WKSP
...
*   attach to dmq queue
    ...
    CALL 'ICA_DMQ_ATTACH_QUE_AND_SBS'  USING  ICA_DMQ_ATTACH_QUE_AND_SBS_WKSP
...
* Description: Update ctrl status in oper table.
*----------------------------------------------------------------------
 BF-00.
...
*   Set ctrl status code to EXECUTE.
    MOVE V$_BATCH_EXECUTE 
      TO CTRL_STATUS_CODE   IN ICA_SVF_HANDLE_OPER_WKSP
...
*----------------------------------------------------------------------
 BG-00.
    ...
    ELSE
       DISPLAY "Error in Sec:BG-GET-CURRENT-TIME  Rtn:UTL_GET_DATE_TIME"
...
*----------------------------------------------------------------------
 BI-00.
    ...
    SET L$_SW01_600_DATA_NOT_FOUND    TO TRUE
...
*----------------------------------------------------------------------
 BIA-00.
    ...
    INITIALIZE  ICD_TRT_600_TRF_MGNT_DBW
...
*   Decrease 1 day from start-date.
    ...
    MOVE "-"        
      TO SP$_MATH_OPERATOR  OF UTL_APPLY_DELTA_TIME_V2_WKSP
...
*----------------------------------------------------------------------
 BIBA-00.
    ...
    INITIALIZE ICD_SVT_036_OPER_DBW
       ...
       WHEN OTHER
	     ...
	     CALL 'ICA_ICF_XLATE_MSG' USING	UTL_CONTROL_ACW
          ...
          ELSE
             ...
             MOVE "SEC:BIBA-CHECK-036 RTN:ICD_SVT_036_OPER_DBA"
               TO SP$_ACW_FREE_TEXT
          END-IF
...
*----------------------------------------------------------------------
 BJ-00.
    ...
    IF DP$_STATUS IN ICD_SVT_030_ENTITY_DBW  NOT = SP$_MSG_NORMAL
	  ...
	  MOVE DP$_STATUS_AUXIL	      IN  ICD_SVT_030_ENTITY_DBW 
            TO SP$_EXIT_STATUS
       END-IF
       ...
       DISPLAY "  DP$_STATUS_AUXIL_TEXT=",
                DP$_STATUS_AUXIL_TEXT IN ICD_SVT_030_ENTITY_DBW CONVERSION
...
*   get entity oper information. (File Name).
    ...
    IF DP$_STATUS  IN ICD_SVT_035_OPER_PRM_DBW  IS FAILURE
       MOVE DP$_STATUS		    IN ICD_SVT_035_OPER_PRM_DBW 
         TO SP$_ACW_PROC_AUX_STATUS 
       ...
       MOVE DP$_STATUS_AUXIL IN ICD_SVT_035_OPER_PRM_DBW 
         TO SP$_EXIT_STATUS
...
*   Read entity param values.
    ...
    CALL 'ICA_SVF_HANDLE_PARAM_VALUE'  USING  ICA_SVF_HANDLE_PARAM_VALUE_WKSP
					      UTL_CONTROL_ACW
    ...
    THEN
	 ...
	 TO SP$_ACW_FORM_AUX_STATUS  IN UTL_CONTROL_ACW
       ...
       DISPLAY "Error in Sec:BJ-GET-ENTITY-INFO "
               "Rtn:ICA_SVF_HANDLE_PARAM_VALUE" 
       DISPLAY "STTS=",
...
*   Set OPER ID symbol for TRB71I job
    ...
    MOVE "GLOBAL"
      TO SP$_SYMBOL_TABLE   IN UTL_SET_SYMBOL_VALUE_WKSP
...
*   600 הלבטמ סנכנ ץבוק יטרפ תאירק 
    ...
    MOVE TRNF_ID    IN ICD_TRT_600_TRF_MGNT_PRW
      ...
      TO TRNF_ID    IN ICA_SVM_CHECK_DUP_BALANC_WKSP
         TRNF_ID    IN ICA_SVM_FILEIN_UPDATE_WKSP
    ...
    IF DP$_STATUS IN ICD_TRT_600_TRF_MGNT_DBW = SP$_MSG_NORMAL
       DISPLAY "Source of the input file :" 
    ...
    ELSE
       ...
       IF DP$_STATUS IN ICD_TRT_600_TRF_MGNT_DBW NOT FAILURE
...
*----------------------------------------------------------------------
 CA-00.
    ...
    MOVE L$_WS01_TOTAL_OPEN_BALANCE	IN K_TRB71I_ADC_REC
      TO L$_WS01_TOTAL_OPEN_BALANCE_9
...
*   תיחכונה המושרה תרימש
 ...
 DA-EXIT.     EXIT.
*----------------------------------------------------------------------
 DAA-CHECK-INP-REC-FIELDS            SECTION.
*----------------------------------------------------------------------
 DAA-00.
    ...
    IF (L$_WS01_CUST_EXT_ID_TYP	 IN K_TRB71I_ADC_REC	IS NUMERIC)  AND
       ...
       (L$_WS01_TOTAL_OPEN_BALANCE_9			IS NUMERIC)  AND
...
*   הייבגב חוקלה לש תילמינימה בוחה תרתי
    IF L$_WS01_MIN_CUST_ADC_BALANCE > 
       L$_WS01_TOTAL_OPEN_BALANCE_9 -
       L$_WS01_TOTAL_BAD_DEBT_9	     
    THEN
       COMPUTE L$_WS01_MIN_CUST_ADC_BALANCE = 
...
*   הניקת םינותנ המושר
    IF L$_SW01_DATA_REC_OK
       ...
       END-IF
    ELSE
       ADD 1  TO L$_CT01_INP_DATA_REC_NOT_OK 
...
*   ATR-ל המושר תביתכ
    IF (L$_MH01_PREV_REC_TYPE_DATA)	    AND
...
*----------------------------------------------------------------------
 DCA-00.
    ...
    IF L$_SW01_DATA_REC_OK
       ...
       THEN
          SET L$_WS01_CAL_STATUS_COMPLETED  IN L$_WS01_PREV_K_TRB71I_ADC_REC  
	   TO TRUE
       ELSE
    ...
    CALL 'ICA_TRB71I_WRITE_ATR_REC'  USING  UTL_CONTROL_ACW
...
*         םירבוצה תורתי ןוכדע
          PERFORM DDA-SET-DBCR-STOP-AMOUNTS
...
*   ילילש רפסמכ גצוימ היבגב חוקלה לש בויחה םוכס 
    COMPUTE TRN_CREDIT_AMOUNT   IN ICA_TRB71I_SET_DBCR_STOP_INP = 
    ...
    EVALUATE SP$_ACW_PROC_AUX_STATUS  OF UTL_CONTROL_ACW
       WHEN SP$_MSG_NORMAL
...
*      תרתוכ תרוש
    ...
    PERFORM MAA-HANDLE-MSM-COMMON-DATA
...
*----------------------------------------------------------------------
 MAA-00.
    ...
    IF L$_SW01_DATA_REC_NOT_OK
       SET L$_WS01_REC_STAT_NOT_OK  IN ICA_TRB71I_MSM_MGS_REC  TO TRUE
    ELSE
    ...
    MOVE L$_WS01_TRX_AMOUNT		IN L$_WS01_PREV_K_TRB71I_ADC_REC
    ...
    MOVE L$_WS01_TRX_DATE_YYYY		IN L$_WS01_PREV_K_TRB71I_ADC_REC
...
*----------------------------------------------------------------------
 UA-00.
    ...
    MOVE SP$_DATE_TIME_BINARY	    IN UTL_GET_DATE_TIME_WKSP
      TO TRNF_INPUTING_TMSP	    IN ICA_SVM_FILEIN_UPDATE_WKSP
...
*----------------------------------------------------------------------
 US-00.
    ...
    IF SP$_SYSPRO_STATUS  OF UTL_SET_SYMBOL_VALUE_WKSP IS FAILURE
...
*----------------------------------------------------------------------
 X4-00.
    ...
    IF DP$_STATUS  IN ICD_COMMON_DBW  IS FAILURE
       ...
       DISPLAY "Error in Sec:X4-ROLLBACK "
               "RTN:ICD_COMMON_DBA" 
       DISPLAY " STTS--->"
                DP$_STATUS	    IN ICD_COMMON_DBW   CONVERSION
...
*----------------------------------------------------------------------
 X-00.
    ...
    DISPLAY "Attention: Program ICA_TRB71I_APPL.COB ended with errors!!!"
...
*----------------------------------------------------------------------
 ...
 Z-EXIT.     EXIT.
*----------------------------------------------------------------------
 E-PROCCESSING-COMPLETE	    SECTION.
*----------------------------------------------------------------------
 E-00.
...
*      ATR ץבוקל םינותנ תמושר תביתכ
       ...
       DISPLAY L$_CT01_INP_TRL_REC         " :טלקה ץבוקב תמויס תומושר כהס"
    ...
    IF INPUT_FILE_STEP_CODE	    IN ICD_TRT_600_TRF_MGNT <= 4
       DISPLAY '<<< ל"אוד תועדוהו תוחוד - 4 בלש ליחתה >>>'
...
*----------------------------------------------------------------------
 ECB-00.
    ...
    STRING
       L$_P_TRB71I_MSM_SUM_MSG_NAME	DELIMITED BY SIZE
       ' '				DELIMITED BY SIZE
...
*   Set send message symbol.
    ...
    MOVE SEND_TERMINATE_MSG	IN ICD_SVT_035_OPER_PRM
      TO SP$_SYMBOL_VALUE	IN UTL_SET_SYMBOL_VALUE_WKSP
...
*----------------------------------------------------------------------
 EH-00.
    ...
    MOVE L$_WS01_TRNF_ID        IN L$_K_TRB71I_ADC_NAME
      TO TRNF_ID		IN ICA_TRB71I_UPD_TABLES_INP
...
*----------------------------------------------------------------------
 ZZC-00.
    ...
    IF DP$_STATUS  IN UTL_DBA_UNIV_WKSP  IN ICD_CONNECT_DBW  IS NOT SUCCESS
       MOVE DP$_STATUS		    IN UTL_DBA_UNIV_WKSP 
				    IN ICD_CONNECT_DBW 
         TO SP$_ACW_PROC_AUX_STATUS 
